...
- pour le §8 (null-narrowing des templates de la PR #5122), ne retenir que les lignes marquées comme non neutres : nœuds DOM devenus conditionnels (`lu-user-tile`, `lu-page-header`, `lu-fieldset`, `lu-color-input`, `lu-comment`, `lu-activity-feed-step`, `segmented-control*`) et gardes sur parent injecté (`lu-data-table-row`, `multilanguage-input`) — chercher les sélecteurs de tests e2e / snapshots / règles CSS qui dépendent de ces nœuds.
