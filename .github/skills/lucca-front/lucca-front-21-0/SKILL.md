...
1. en priorité `node_modules/@lucca-front/ng/package.json` → champ `version` (version résolue exacte, ex: `21.0.5`) ;
2. à défaut, la dépendance `@lucca-front/ng` (ou `@lucca-front/scss`) dans le `package.json` du projet (ex: `^21.0.5` → `21.0.5`).

La documentation `references/` reflète le **dernier patch publié : 21.0.5**. Si le patch du projet est **antérieur**, les correctifs livrés après sa version sont décrits dans `fixes/` (voir §2) — ils ne sont **pas** dans son code : consulte tous les `fixes/<M-m-p>.md` de version **strictement supérieure** au patch installé et ignore ces changements.
...
- le **patch** détecté est **postérieur** à 21.0.5 (dernier patch connu de cette skill → skill périmée, l'API réelle peut différer) ;
...
À consulter quand : le projet est sur un patch **antérieur** à 21.0.5 (les fixes postérieurs à sa version décrivent des correctifs absents de son code), ou pour comprendre ce qu'un patch précis a changé.
...
| Projet sur un patch antérieur à 21.0.5 / comportement inattendu sur un patch | `./fixes/<M-m-p>.md` |
