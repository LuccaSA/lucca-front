...
1. en priorité `node_modules/@lucca-front/ng/package.json` → champ `version` (version résolue exacte, ex: `21.2.5`) ;
2. à défaut, la dépendance `@lucca-front/ng` (ou `@lucca-front/scss`) dans le `package.json` du projet (ex: `^21.2.5` → `21.2.5`).

La documentation `references/` reflète le **dernier patch publié : 21.2.5**. Si le patch du projet est **antérieur**, les correctifs livrés après sa version sont décrits dans `fixes/` (voir §2) — ils ne sont **pas** dans son code : consulte tous les `fixes/<M-m-p>.md` de version **strictement supérieure** au patch installé et ignore ces changements.
...
| Projet sur un patch antérieur à 21.2.5 / comportement inattendu sur un patch | `./fixes/<M-m-p>.md` |
