...
Cette skill couvre **Lucca Front 21.3.x** (patchs publiés : 21.3.0, 21.3.1). C'est la mineure installée sur le projet — tous les chemins ci-dessous lui sont relatifs. Il n'y a rien à détecter.

La documentation `references/` reflète le **dernier patch publié : 21.3.1**. Si le projet est sur un patch antérieur, les correctifs livrés après sa version sont décrits dans `fixes/` (voir §2) — ils ne sont **pas** dans son code.
