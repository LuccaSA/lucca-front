# fileupload — Design Guidelines

> Sourced from [Prisme / ZeroHeight](https://prisme.lucca.io/94310e217/p/8282a5)
...
Lorsqu’un fichier est importé, il est représenté par le composant FileEntry en lieu et place de la zone de sélection de fichier. Une miniature permet d'afficher une icône indiquant le format du fichier chargé (pdf, csv, xls, image, etc.).

Si l’utilisateur souhaite changer de fichier, il doit supprimer le fichier chargé pour en sélectionner un nouveau.

Lorsque le contexte requiert l'affichage et l'aperçu du fichier importé, l'activation de la propriété `media` permet au FileEntry d'afficher le contenu de l'image.

À noter que si l’affichage du fichier requiert une navigation entre plusieurs pages ou encore une gestion du zoom, il s’agit alors d’un composant autre que FileEntry.
...
Un affichage spécifique en grille est possible quand on attend de l’utilisateur qu’il importe des images. La propriété `media` permet de donner un aperçu plus explicite des images sélectionnées et potentiellement de les réordonner.
