...
~ `useAutoPrefixSuffix` : transform booleanAttribute → luBooleanAttribute
~ `min` : number | undefined → unknown, transform ∅ → luOptionalNumberAttribute
~ `max` : number | undefined → unknown, transform ∅ → luOptionalNumberAttribute
