...
import { ChangeDetectionStrategy, Component, computed, inject, LOCALE_ID, Pipe, PipeTransform, signal } from '@angular/core';
...
export class DataTableComponent {
	...
	addTask(task: Task): void {
		let groupName = formatDate(task.dueDate, 'MMMM y', this.locale);
		groupName = groupName[0].toUpperCase() + groupName.slice(1);

		const existingGroups = this.groups();
		const existingGroup = existingGroups.find((grp) => grp.groupName === groupName);

		if (existingGroup) {
			const newGroup: TaskGroup = { ...existingGroup, tasks: [...existingGroup.tasks, task], isExpanded: true };
			this.groups.set(existingGroups.map((grp) => (grp === existingGroup ? newGroup : grp)));
		} else {
			const newGroup: TaskGroup = { groupName: groupName, isExpanded: true, tasks: [task] };
			this.groups.set([...existingGroups, newGroup]);
		}
	}

	removeTask(taskGroup: TaskGroup, task: Task): void {
		const updatedGroup: TaskGroup = {
			...taskGroup,
			tasks: taskGroup.tasks.filter((t) => t !== task),
		};

		if (updatedGroup.tasks.length === 0) {
			this.groups.set(this.groups().filter((grp) => grp !== taskGroup));
		} else {
			this.groups.set(this.groups().map((grp) => (grp === taskGroup ? updatedGroup : grp)));
		}
	}
}
