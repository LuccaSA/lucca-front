	...
	template: `
		<div class="multipleSelect-displayer">
			...
			@for (option of displayedOptions$ | async; track option; let index = $index) {
				<lu-chip class="multipleSelect-displayer-chip palette-product" withEllipsis (kill)="unselectOption(option, $event)" [unkillable]="select.disabled$ | async">
					<ng-container *luOptionOutlet="select.displayerTpl(); value: option" />
				</lu-chip>
			}
			@if (overflowOptions$ | async; as overflow) {
				<lu-chip class="multipleSelect-displayer-chip palette-product" unkillable>+ {{ overflow }}</lu-chip>
			}
		</div>
	`,
