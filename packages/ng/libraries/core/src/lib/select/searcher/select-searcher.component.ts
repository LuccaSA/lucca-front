	...
	providers: [
		{
			...
			useExisting: forwardRef(() => LuSelectSearcherComponent),
		},
	],
})
/**
 ...
 */
export class LuSelectSearcherComponent<T> extends ASelectOptionFeeder<T>
	implements ISelectSearcher<T>, OnDestroy, AfterContentInit {
	_clue = '';
	_noResults = false;
	...
	filter(clue: string, options: LuSelectOption<T>[]): LuSelectOption<T>[] {
		...
		return options
			...
			.filter(option => {
				...
				if (!valueOption) {
					this.innerMap[option.viewValue] = this._normalizeString(
						option.viewValue,
					);
