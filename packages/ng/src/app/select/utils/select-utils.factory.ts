...
export function sameOption<T>(oldItem: T, newItem: T): boolean {
	 ...
	 */
export function findOption<T>(optionArray: LuSelectOption<T>[], option: T): LuSelectOption<T> {
	return optionArray.find(selectOption => sameOption(selectOption.luOptionValue, option));
}
