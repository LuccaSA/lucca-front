...
describe('LuUserSelect', () => {
	...
	it('It should call the user service at first', () => {
		...
		fixture.whenStable().then(() => {
			...
			const testRequest = http.expectOne(
				'/api/v3/users/find?formerEmployees=false&clue=&paging=0,10&fields=id,firstName,lastName',
			);
			expect(testRequest.request).toBeDefined(
				'The users sould be called with correct parameters',
			);
		});
	});
	...
	it('It should reference has the right value', () => {
		...
		const userSelect: LuUserSelect<any> = fixture.debugElement.query(
			By.directive(LuUserSelect),
		).componentInstance;
		fixture.detectChanges();
