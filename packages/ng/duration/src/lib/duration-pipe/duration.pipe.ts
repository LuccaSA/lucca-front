...
export class LuDurationPipe implements PipeTransform {
  ...
  public transform(
    ...
    if (unit === 'day' && decimalPrecision !== undefined) {
      ...
      return `${sign}${calculatedDays.toFixed(decimalPrecision)}\u00a0j`;
      ...
      const calculatedSeconds =
        parsedSeconds +
        (displayUnitsArray.includes('min')
          ? 0
          : calculatedMinutes * 60);

      return `${sign}
				${displayUnitsArray.includes('h') ? `${calculatedHours.toLocaleString(this.locale, { minimumIntegerDigits: 1 })}\u00a0h${displayUnitsArray.includes('min') || displayUnitsArray.includes('s') ? '\u00a0' : ''}` : ''}
				${displayUnitsArray.includes('min') ? `${calculatedMinutes.toLocaleString(this.locale, { minimumIntegerDigits: 2 })}\u00a0min${displayUnitsArray.includes('s') ? '\u00a0' : ''}` : ''}
				${displayUnitsArray.includes('s') ? `${calculatedSeconds.toLocaleString(this.locale, { minimumIntegerDigits: 2 })}\u00a0s` : ''}`;
    } else {
      return '';
    }
  }
