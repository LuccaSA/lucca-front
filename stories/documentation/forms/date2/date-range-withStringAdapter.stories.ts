	...
	render: (args, { argTypes }) => {
		...
		return {
			...
			template: cleanupTemplate(`<lu-form-field label="Date range input example" inlineMessage="Inline message example">
				<lu-date-range-input withStringAdapter [(ngModel)]="selected" [min]="min" [max]="max" ${generateInputs(flags, argTypes)}></lu-date-range-input>
			</lu-form-field>
...
export const DateRangeWithStringAdapter: StoryObj<DateRangeInputComponent> = {
	args: {
		hideToday: false,
		hideWeekend: false,
		clearable: false,
